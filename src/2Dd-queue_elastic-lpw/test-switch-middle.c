...
#if !defined(VALIDATE_WINDOW)
	#define VALIDATE_WINDOW 1
#endif
...
uint64_t depth = 1;
uint32_t max_width;
uint64_t elastic_depth;
uint32_t elastic_width;
uint8_t k_mode = 0;
size_t side_work = 0;
...
void* test(void* thread)
{
	...
	// Initialize the windows to a valid starting state?
	// thread_PWindow = global
	...
	#if VALIDATE_WINDOW == 1
	if (!thread_id)
	{
		// EL: Validate that all structures are in their windows
...
{
	assert(sizeof(node_t) == CACHE_LINE_SIZE);
    ...
    {
		i = 0;
		c = getopt_long(argc, argv, "hAf:d:i:n:r:u:m:a:l:p:v:f:y:z:k:w:s:c:e:b:", long_options, &i);
		if(c == -1)
		break;
	...
	if (elastic_width > width && max_width == 0) {
	...
	// EL: Added a new parameter to this, but should any way just be used for the elastic one.
	...
	if (elastic_depth != 0 || elastic_width != 0)
	{
	...
	int UNUSED pr = (int) (putting_count_total_succ - removing_count_total_succ);
		...
		{
			uint64_t diff = initial + pr > size_after ? (initial + pr)-size_after : size_after - (initial + pr);
			printf("\n******** ERROR WRONG size. %zu + %d != %zu (difference %zu)**********\n\n", initial, pr, size_after, diff);
	...
	if (elastic_width != 0){
		...
		assert(get_width(set) == elastic_width);
	...
	if (elastic_depth != 0){
		...
		assert(get_depth(set) == elastic_depth); // printf("Elastic depth , %zu\n", elastic_depth);
